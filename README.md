...
curl -L "https://drive.google.com/file/d/1GRkD-maXmrRjy8Uh80sfYqN21gqFROi3/view?usp=sharing" -o runs/baseline12/best.pt
...
| 이름 | 역할 | 주요 기여 |
|---|---|---|
| TBD | 데이터/학습 | ETRI 전처리, 12클래스 매핑, R3D-18 학습/평가 |
| TBD | 대시보드/통합 | Streamlit UI, YOLO/행동/낙상 통합, 데모 안정화 |
| TBD | 실험/문서 | 비교 모델, URFD 낙상 실험, README/발표 자료 정리 |

---

## 참고 자료

- `9시스템통합.pdf`: MVP 범위 조정, 후처리/안정화, 재현성, README 필수 항목
- `10최종리허설.pdf`: 문제→접근→결과→데모→한계 스토리라인, 네트워크 독립성, 최종 리허설 체크리스트
